...
public class FirstDuplicateUsingTwoForLoops {
  ...
  private static int firstDuplicate(int[] a) {
    ...
    int minIndexOfDupSecondValue = a.length;

    for (int i = 0; i < a.length; i++) {
      for (int j = i + 1; j < a.length; j++) {
        if (a[i] == a[j]) {
          minIndexOfDupSecondValue = Math.min(minIndexOfDupSecondValue, j);
        }
      }
    }

    if (minIndexOfDupSecondValue == a.length)
      return -1;
    else
      return a[minIndexOfDupSecondValue];
  }
